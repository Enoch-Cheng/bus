...
function initTable(id, settings) {
  ...
  for (i = settings.list.length - 1; i >= 0; i--) {
    ...
    var cell = row.insertCell(0);
    cell.innerHTML = settings.list[i].stopname;
    var cell = row.insertCell(0);
    cell.innerHTML = settings.list[i].route;
  }

  var header = table.createTHead();
  row = header.insertRow(0);
  cell = row.insertCell(0);
  cell.innerHTML = "<b>Estimated time of arrival</b>";
  cell = row.insertCell(0);
  cell.innerHTML = "<b>Stop</b>";
  cell = row.insertCell(0);
  cell.innerHTML = "<b>Route</b>";
...
async function getKmb_eta_old(d, idx, wait) {
  const url =
    "http://etav3.kmb.hk/?action=geteta&lang=tc" +
    "&route=" +
    d.route +
    "&bound=" +
    d.bound +
...
function parseKMB(eta) {
  ...
  for (i = 0; i < eta.length; i++) {
    if (i == eta.length - 1) {
      string = string + formatTime(eta[i].eta) + " " + eta[i].rmk_tc;
    } else {
      string = string + formatTime(eta[i].eta) + " " + eta[i].rmk_tc + "<br>";
    }
  }
  return string;
}

function parseKMB_old(eta) {
  var string = "";
  for (i = 0; i < eta.length; i++) {
    if (i == eta.length - 1) {
